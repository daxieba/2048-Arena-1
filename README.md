...
### 进阶功能
- 🤖 **AI 自动玩**：Expectimax 搜索 + 启发式评分，每 1.5 秒自动走一步，随时停止；支持 **普通 / 大师** 两档强度
...
点击工具栏"🤖 自动玩"，AI 每 **1.5 秒**决策并移动一次。自动玩前可通过下拉选择强度：

- **普通（normal）**：4×4 搜索深度 4，上手快，成绩不错
- **大师（master）**：每档深度 +1（4×4 深度 5），思考更深，容易打出更高分（实测可稳定合出 512~1024）

> 自动玩期间强度下拉会禁用，停止后恢复；强度选择会持久化保存。
